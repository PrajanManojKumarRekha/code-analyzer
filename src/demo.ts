...
function parseCliArgs(args: string[]): { mode: "mock" | "mcp" | "live"; targetDir: string; question: string } {
  let mode: "mock" | "mcp" | "live" = "mock";
    ...
    while (toolCalls.length > 0) {
      ...
      for (const call of toolCalls) {
        if (call.name === "read_file") {
          const args = call.args as { relativePath: string; symbolsOnly?: boolean };
          console.log(`       -> Agent calling read_file("${args.relativePath}")`);
          
          try {
            const readResult = readFile(path.join(TARGET_DIR, args.relativePath), {
              baseDir: TARGET_DIR,
